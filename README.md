...
의료진과 환자를 위한 **차세대 Flask 기반 웹 애플리케이션**으로, 부비동 X-ray 영상을 업로드하면 **스마트 재분류 AI 모델**이 정밀 진단을 수행하고, **실제 GradCAM 히트맵**과 **Z-score 통계 분석**을 통해 의학적 근거를 시각화하며, 4탭 인터페이스를 통해 **AI 진단**, **근거 분석**, **의료진 판독**, **AI 상담** 서비스를 제공합니다.
...
### 🎯 **차세대 스마트 재분류 AI 시스템**
- **스마트 재분류**: "Both" → "Left/Right-Dominant" 자동 수정으로 진단 정확도 대폭 향상
- **Normal→병변 감지**: 놓친 병변 자동 탐지 (15% 임계값) - "Normal" 오진단 방지
